...
class HoldReadyWatcher(watcher_v5.PreLiveWatcher):
    ...
    def _hold_session_ready(self) -> bool:
        if self.session_ok is not True:
            # Make sure a prove-only health check runs as soon as the current
            # poll finishes. _start_session_worker itself refuses to start while
            # a reservation attempt is fenced by self.holding.
            if self.session_check_every:
        ...
        # Never trust a successful proof indefinitely. If the health worker has
        # been unable to renew it for two normal check intervals, gate holding
        # until a new strong proof succeeds.
    ...
    def _poll_session_worker(self) -> None:
        ...
        if not definitive_expiry:
            ...
            # Keep a still-fresh prior proof, or remain gated if startup has not
            # been proven yet, and retry the harmless proof soon.
            log.warning(
        ...
        # A prove-only worker was copied from the live browser and was actually
