...
def test_fresh_proof_reuses_natural_auth_return_candidate_2xx(monkeypatch):
    page = ReloadingPage()
    ...
    monkeypatch.setattr(session_proof.login_flow, "StateDetector", Detector)
    monkeypatch.setattr(session_proof.site_selectors, "is_login_page", lambda _page: False)
    probe = session_proof._ProtectedBackendProbe("hiring.amazon.ca")
    probe.attach(page)
    page._emit_candidate(200)
    ...
    assert proof.passed is True
    assert proof.application_backend_authenticated is True
    assert "during auth return" in proof.reason
    assert page.reloads == 0
    assert page.gotos == []


def test_inconclusive_early_403_still_runs_existing_reload_proof(monkeypatch):
    page = ReloadingPage()
...
def test_early_candidate_401_fails_without_retrying_a_reload(monkeypatch):
    page = ReloadingPage()
